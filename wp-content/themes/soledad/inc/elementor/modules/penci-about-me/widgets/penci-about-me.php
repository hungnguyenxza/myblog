...
class PenciAboutMe extends Base_Widget {
	...
	protected function _register_controls() {
		...
		$this->end_controls_section();
		$this->register_block_title_style_section_controls( false );
	...
	protected function render() {
		...
		$image_src = Group_Control_Image_Size::get_attachment_image_src( $settings['image'], 'thumbnail', $settings );
			...
			<div class="penci-block_content about-widget<?php if ( $settings['align_block'] ): echo ' pc_align' . esc_attr( $settings['align_block'] ); endif; ?>">
				<?php if ( $image_src ) : ?>
					...
					<?php if ( ! $settings['dis_lazyload'] ) { ?>
						<img class="penci-widget-about-image nopin holder-square penci-lazy" nopin="nopin" src="<?php echo get_template_directory_uri() . '/images/penci2-holder.png'; ?>" data-src="<?php echo esc_url( $image_src ); ?>" alt="<?php echo esc_attr( $settings['about_us_heading'] ); ?>"/>
					<?php } else { ?>
						<img class="penci-widget-about-image nopin holder-square" nopin="nopin" src="<?php echo esc_url( $image_src ); ?>" alt="<?php echo esc_attr( $settings['about_us_heading'] ); ?>"/>
					<?php } ?>
