	...
	class Penci_Vc_Helper {
		...
		public static function get_heading_block_css( $block_id_css, $args ) {
			...
			if ( $r['btitle_shapes_color'] ) {
				...
				$output .= $block_id_css . ' .style-13.pcalign-right .inner-arrow:before { border-left-color: ' . esc_attr( $r['btitle_shapes_color'] ) . '; }';

				$output .= $block_id_css . ' .style-13.pcalign-center .inner-arrow:after,';
				$output .= $block_id_css . ' .style-13.pcalign-left .inner-arrow:after { border-right-color: ' . esc_attr( $r['btitle_shapes_color'] ) . '; }';

				$output .= $block_id_css . ' .style-12 .inner-arrow:before,';
				$output .= $block_id_css . ' .style-12.pcalign-right .inner-arrow:after,';
				$output .= $block_id_css . ' .style-12.pcalign-center .inner-arrow:after{ border-bottom-color: ' . esc_attr( $r['btitle_shapes_color'] ) . '; }';

				$output .= $block_id_css . ' .style-11 .inner-arrow:after,';
				$output .= $block_id_css . ' .style-11 .inner-arrow:before{ border-top-color: ' . esc_attr( $r['btitle_shapes_color'] ) . '; }';
				$output .= $block_id_css . ' .penci-homepage-title{ background-color: ' . esc_attr( $r['btitle_shapes_color'] ) . '; }';
			}

			if ( $r['btitle_bgcolor'] ) {
				$output .= $block_id_css . ' .style-2.penci-border-arrow:after{ border-color: transparent;border-top-color: ' . esc_attr( $r['btitle_bgcolor'] ) . '; }';
				...
				$output .= $block_id_css . ' .penci-border-arrow .inner-arrow{ background-color: ' . esc_attr( $r['btitle_bgcolor'] ) . '; }';
			}
		...
		public static function get_image_holder_gal( $attach_id, $size = 'full', $image_type, $is_background = true, $count = '', $class = '', $caption_source = '' ) {
			...
			if ( $is_background ) {
				...
				<div class="penci-gallery-item penci-galitem-<?php echo $count . ( $class ? ' ' . $class : '' ); ?>">
					<a class="penci-image-holder<?php echo $class_lazy; ?>" <?php echo $data_src; ?> href="<?php echo $src_large; ?>" data-rel="penci-gallery-image-content" <?php echo $gallery_title; ?>>
				...
				ob_start();
				?>
				<a class="<?php echo $class_lazy . ( $class ? ' ' . $class : '' ); ?>" href="<?php echo $src_large; ?>" data-rel="penci-gallery-image-content"<?php echo $gallery_title; ?>>
