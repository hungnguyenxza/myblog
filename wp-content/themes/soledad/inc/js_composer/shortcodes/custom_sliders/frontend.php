			...
			foreach ( (array) $penci_slides as $slide ) {
				...
				if ( isset( $slide['button_text'] ) && $slide['button_text'] ) {
					...
					if ( isset( $atts['button_link'] ) && $atts['button_link'] ) {
						$link_data = self::get_link_button( $atts['button_link'], $come_from );
					}
				...
				if ( isset( $slide['button_text2'] ) && $slide['button_text2'] ) {
					$link_data = 'href="#"';
					if ( isset( $atts['button_link2'] ) && $atts['button_link2'] ) {
						$link_data = self::get_link_button( $atts['button_link2'], $come_from );
					}
