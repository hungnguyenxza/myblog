			...
			<div class="penci-recipe-heading">
				...
				<div class="penci-recipe-metades">
					...
					<?php if ( ! get_theme_mod( 'penci_recipe_print' ) ) : ?>
						<a href="#" class="penci-recipe-print" data-print="<?php echo plugin_dir_url( __FILE__ ) . 'print.css?ver=2.2'; ?>"><?php ( function_exists( 'penci_fawesome_icon' ) ? penci_fawesome_icon( 'fas fa-print' ) : '<i class="fa fa-print"></i>' ); ?> <?php if( get_theme_mod( 'penci_recipe_print_text' ) ) { echo do_shortcode( get_theme_mod( 'penci_recipe_print_text' ) ); } else { esc_html_e( 'Print This', 'soledad' ); } ?></a>
					<?php endif; ?>

					<?php if ( $recipe_servings || $recipe_cooktime || $recipe_preptime ) : ?>
						<div class="penci-recipe-meta">
							<?php if ( $recipe_servings ) : ?><span>
								<?php ( function_exists( 'penci_fawesome_icon' ) ? penci_fawesome_icon( 'fas fa-user' ) : '<i class="fa fa-user"></i>' ); ?> <?php if( get_theme_mod( 'penci_recipe_serves_text' ) ) { echo do_shortcode( get_theme_mod( 'penci_recipe_serves_text' ) ); } else { esc_html_e( 'Serves', 'soledad' ); } ?>: <span class="servings" itemprop="recipeYield"><?php echo
									$recipe_servings; ?></span>
								</span>
							<?php endif; ?>
							<?php if ( $recipe_preptime ) : ?>
								<span>
								<?php ( function_exists( 'penci_fawesome_icon' ) ? penci_fawesome_icon( 'far fa-clock' ) : '<i class="fa fa-clock-o"></i>' ); ?> <?php if( get_theme_mod( 'penci_recipe_prep_time_text' ) ) { echo do_shortcode( get_theme_mod( 'penci_recipe_prep_time_text' ) ); } else { esc_html_e( 'Prep Time', 'soledad' ); } ?>: <time <?php if( $recipe_preptime_fm ): echo 'datetime="PT'. $recipe_preptime_fm .'" '; endif;?>itemprop="prepTime"><?php echo $recipe_preptime; ?></time>
								</span>
							<?php endif; ?>
							<?php if ( $recipe_cooktime ) : ?>
								<span>
								<?php ( function_exists( 'penci_fawesome_icon' ) ? penci_fawesome_icon( 'far fa-clock' ) : '<i class="fa fa-clock-o"></i>' ); ?> <?php if( get_theme_mod( 'penci_recipe_cooking_text' ) ) { echo do_shortcode( get_theme_mod( 'penci_recipe_cooking_text' ) ); } else { esc_html_e( 'Cooking Time', 'soledad' ); } ?>: <time <?php if( $recipe_cooktime_fm ): echo 'datetime="PT' . $recipe_cooktime_fm .'" '; endif;?>itemprop="totalTime"><?php echo $recipe_cooktime; ?></time>
